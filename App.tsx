...
// Since the environment already defines it, we make it optional and use any to avoid conflicts while maintaining access.
declare global {
  interface Window {
    aistudio?: any;
  }
...
const App: React.FC = () => {
  ...
  const startBroadcasting = async () => {
    try {
      ...
      // Rule: Specialized models like Native Audio require a selected API key in this environment.
      if (window.aistudio && !(await window.aistudio.hasSelectedApiKey())) {
        await window.aistudio.openSelectKey();
        // After opening the dialog, we assume selection and proceed to avoid race conditions.
      }
      ...
      const sessionPromise = ai.live.connect({
        ...
        callbacks: {
          ...
          onmessage: async (message: LiveServerMessage) => {
            // Rule: Handle user input transcription
            ...
            // Rule: Must handle audio output from the model turn even if transcription only
            const modelAudio = message.serverContent?.modelTurn?.parts[0]?.inlineData?.data;
            if (modelAudio) {
               // We don't play host audio back to the host, but we must acknowledge receipt
               console.debug("Model audio received in host mode.");
            }
          ...
          onerror: (e) => {
            ...
            // If it's a network error, it's often due to the key selection
        ...
        config: {
          ...
          Respond ONLY with transcription chunks. Do not provide spoken responses.`,
          speechConfig: {
      ...
      setStatus(ConnectionStatus.ERROR);
      if (err.message?.includes("Network error") || err.message?.includes("not found")) {
        window.aistudio?.openSelectKey();
      }
    }
  };
  ...
  const stopBroadcasting = () => {
    // Fix: Properly close the live session to release resources
  ...
  const processTranslationAndSpeech = async (originalText: string) => {
    ...
    try {
      // Create new instance for standalone translation/TTS calls too
      const ai = new GoogleGenAI({ apiKey: process.env.API_KEY });
      ...
      <header className="px-6 pt-10 pb-4 z-20">
        <div className="flex justify-between items-start">
          ...
          <div className="flex items-center bg-zinc-900/90 backdrop-blur border border-zinc-800 rounded-full px-3 py-1 shadow-2xl">
             <div className={`w-2 h-2 rounded-full mr-2 transition-all duration-300 ${status === ConnectionStatus.CONNECTED ? 'bg-emerald-500 shadow-[0_0_8px_#10b981]' : (mode === 'RECEIVER' ? 'bg-blue-500 shadow-[0_0_8px_#3b82f6]' : (status === ConnectionStatus.ERROR ? 'bg-red-500 shadow-[0_0_8px_#ef4444]' : 'bg-zinc-700'))}`} />
            ...
            {transcriptions.length === 0 ? (
              <div className="h-full flex flex-col items-center justify-center opacity-40">
                ...
                <p className="mt-8 text-[11px] font-black tracking-[0.4em] uppercase text-zinc-700 text-center px-12 leading-relaxed">
                  {status === ConnectionStatus.ERROR ? 'Connection issue. Please verify API Key or Channel.' : 
                   (mode === 'SOURCE' ? 'Host, begin your session to capture audio' : 'Participant, awaiting transcription from host')}
                </p>
                {status === ConnectionStatus.ERROR && (
                  <button onClick={() => window.aistudio?.openSelectKey()} className="mt-4 text-[10px] font-black text-orange-500 uppercase tracking-widest underline underline-offset-4">
                    Update API Key
                  </button>
                )}
              </div>
          ...
          status === ConnectionStatus.CONNECTED ? (
            ...
            <button onClick={startBroadcasting} disabled={status === ConnectionStatus.CONNECTING} className="group relative w-full h-18 rounded-3xl overflow-hidden active:scale-95 transition-all disabled:opacity-50">
              ...
              <span className="relative z-10 flex items-center justify-center gap-4 text-[11px] font-black uppercase tracking-[0.4em] text-white">
                ...
                {status === ConnectionStatus.CONNECTING ? 'Connecting...' : 'Start Broadcast'}
              </span>
            </button>
          )
        ) : (
           <div className="h-18 flex items-center justify-between bg-zinc-900/90 backdrop-blur-2xl border border-zinc-800/80 px-8 rounded-3xl shadow-xl">
              <div className="flex flex-col">
                ...
                <span className="text-[10px] font-black text-blue-500 uppercase tracking-tight">Participant Sync Active</span>
              </div>
